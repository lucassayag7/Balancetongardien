...
export async function POST(request: NextRequest) {
  try {
    ...
    // S'assurer que l'utilisateur existe dans Prisma (auto-création si besoin)
    await prisma.user.upsert({
      ...
      create: {
        ...
        pseudo: authUser.user_metadata?.pseudo ?? authUser.email?.split("@")[0] ?? authUser.id.slice(0, 8),
      },
    });
    ...
    await prisma.buildingUser.upsert({
      where: { buildingId_userId: { buildingId: building.id, userId: authUser.id } },
      update: {},
      create: { buildingId: building.id, userId: authUser.id },
    });
    ...
    return NextResponse.json(
      { error: e instanceof Error ? e.message : "Erreur serveur lors de la création du signalement." },
      { status: 500 }
    );
  }
}
